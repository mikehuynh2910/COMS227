	...
	public ConnectGame(int width, int height, int min, int max, Random rand) {
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                Tile tile = new Tile(rand.nextInt(max - min + 1) + min);
...
//TODO
	...
	public Tile getRandomTile() {
		...
		int level =  random.nextInt(MAX_TILE_LEVEL - MIN_TILE_LEVEL + 1);
		return tile;
	}
	...
	public boolean tryFirstSelect(int x, int y) {
		if (tile.setLocation(x,y).getLevel() != MIN_TILE_LEVEL) {
			return;
		}
		return false;
	}
...
//		int newLevel = lastTile.getLevel() + 1;
//		lastTile.setLevel(newLevel);
//		lastTile.setSelect(false);
//		score += selectedTiles.size() * lastTile.getValue();
//		upgradeLastSelected(newLevel);
//		dropSelected();
//		dropLevel();
//		clearSelectedTiles();
	}
